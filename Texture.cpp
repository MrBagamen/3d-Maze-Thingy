...
void Texture::LoadTexture(GLuint _tex, bool anisotropicFiltering, GLfloat afLevel, GLfloat tx, GLfloat ty)
{
    tex = _tex;
    texcoord[0] = 0.0f;texcoord[1] = 0.0f;
    texcoord[2] = tx;texcoord[3] = 0.0f;
    ...
    if(anisotropicFiltering)
    {
        if(GLEW_EXT_texture_filter_anisotropic)
        {
            glTexSubImage2D(GL_TEXTURE_2D, 0, GL_RGB, w, h, 0, GL_RGB, GL_UNSIGNED_BYTE, pixels);
            glGenerateMipmap(GL_TEXTURE_2D);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, afLevel);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR_MIPMAP_LINEAR);
            glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
